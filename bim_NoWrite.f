...
c     2 Aug 2011
c     Write statements during execution have been commented out; however, incorrect
c     input parameter writes have been left in.  Search for "jjv"
...
C
      ...
      real(8) t0vec(1) ! jvallino 26 sep 08
C
C OUTPUT PARAMETERS
C
...
c          WRITE(6,900) T0
      ...
      ! jvallino 26 Sep 08: this was changed to prevent compiler errors.
...
!     &  CALL SOLOUT(M,T0,Y0,F0,1,ORD,IRTRN)
      IF(IOUT.EQ.1) then
        t0vec(1) = t0  
        CALL SOLOUT(M,t0vec,Y0,F0,1,ORD,IRTRN)
      end if
...
c jjv              
c                 WRITE(6,1040) IERR
c 1040            FORMAT(/,/,'ERROR CODE IERR = ', I3, /,
c     &           'RETURNED BY THE SUBROUTINE JAC',/,/)
c                 WRITE(6,900) T0
                 IDID = -6
                 GOTO 800
              END IF
